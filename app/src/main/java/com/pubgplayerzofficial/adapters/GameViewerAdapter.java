...
public class GameViewerAdapter extends RecyclerView.Adapter<GameViewerAdapter.MyClass> {
    ...
    public GameViewerAdapter(Context context, List resultList) {
    ...
    @NonNull
    @Override
    public GameViewerAdapter.MyClass onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_gemeviewer, viewGroup, false);
    ...
    public void onBindViewHolder(@NonNull final MyClass myClass, final int i) {
        ...
        final int uspace = resultList.get(i).getTotalused();
        final int total = resultList.get(i).getSlot();
        ...
        if (total <= uspace) {
            ...
            myClass.bt_submit.setBackground(context.getResources().getDrawable(R.drawable.done_corner_round_acent));
            myClass.bt_submit.setTextColor(context.getResources().getColor(R.color.colorBlack));
        } else {
...
//                    Toasty.success(context, "Success").show();
    ...
    private void checkJoin(int i, int total, int userd) {
        ...
        if (result != null) {
            if (result.getWallet_amount() >= resultList.get(i).getEntry_fee()) {
                ...
                JoinMatchFragment joinMatchFragment = JoinMatchFragment.newInstance(result.getWallet_amount(), resultList.get(i).getEntry_fee());
        ...
        btn_join.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (edit_username.getText().toString().length() == 0) {
                    ...
                    checkPubgName(dialog, edit_username, i);
                }
            }
        });
    }

    private void checkPubgName(final Dialog dialog, final EditText edit_username, final int i) {
        DbHelper dbHelper = new DbHelper(context);
        ...
        //check slot
        serviceCaller.callPCheckSlotService(resultList.get(i).getId(), new IAsyncWorkCompletedCallback() {
            @Override
            public void onDone(String workName, boolean isComplete) {
                if (isComplete) {
                    if (resultList.get(i).getSlot() > Integer.parseInt(workName.trim())) {
